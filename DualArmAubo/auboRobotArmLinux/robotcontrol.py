...
class Auboi5Robot:
        ...
        if self.rshd >= 0:
            if not self.connected:
                if libpyauboi5.login(self.rshd, ip, port) == 0:
                    self.connected = True
                    time.sleep(0.5)
                    logger.info("连接成功,robot_ip:{},port:{}".format(ip, port))
                    return RobotErrorType.RobotError_SUCC
                else:
    ...
    def get_joint_maxacc(self):
        ...
        * NOTES:
        ...
        if self.rshd >= 0 and self.connected:
            ...
            logging.info("六个关节的最大加速度{}".format(result))
            return result
        else:
            logger.warn("RSHD uninitialized or not login!!!")
            return None
    ...
    def get_end_max_line_acc(self):
        ...
        * NOTES:
        ...
        if self.rshd >= 0 and self.connected:
            ...
            logging.info("机械臂末端最大线加速度{}".format(result))
            return result
    ...
    def get_end_max_line_velc(self):
        ...
        * NOTES:
        ...
        if self.rshd >= 0 and self.connected:
            ...
            logging.info("机械臂末端最大线速度{}".format(result))
            return result
    ...
    def get_end_max_angle_acc(self):
        ...
        * NOTES:
        ...
        self.check_event()
        if self.rshd >= 0 and self.connected:
            result = libpyauboi5.get_end_max_angle_acc(self.rshd)
            logging.info("机械臂末端最大角加速度{}".format(result))
            return result
    ...
    def get_end_max_angle_velc(self):
        ...
        * NOTES:
        ...
        self.check_event()
        if self.rshd >= 0 and self.connected:
            result = libpyauboi5.get_end_max_angle_velc(self.rshd)
            logging.info("机械臂末端最大线速度{}".format(result))
            return result
            # return libpyauboi5.get_end_max_line_velc(self.rshd)
        else:
    ...
    def move_to_target_in_cartesian(self, pose):
        """
        ...
        * NOTES:
        ...
        if self.rshd >= 0 and self.connected:
            ...
            # print("ik_result[1]====>{}".format(ik_result))  
    ...
    async def move_to_target_in_cartesian_async(self, pose):
        """
        * FUNCTION:    move_to_target_in_cartesian
        * DESCRIPTION: 给出笛卡尔坐标值和欧拉角，机械臂轴动到目标位置和姿态
        * INPUTS:      pos:位置坐标（x，y，z），单位(m)
