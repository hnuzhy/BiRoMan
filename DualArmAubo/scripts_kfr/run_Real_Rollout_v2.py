...
from src.utils import cfg_dict_init, calInsideRectIOU
from src.utils import interpolate_se3_bspline_startend_poses
from src.utils import compute_rotation_by_image_moments

from kingFisherR.utils_vlms import conduct_object_detect_and_segment
from kingFisherR.utils_vlms import plot_processed_results_vis
...
################################################
python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name plugpen --marker_id 1
python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name reorient --anyobj_id 1
python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name unscrew --bottle_id 6
python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name pouring --bottle_id 6 --mugcup_id 1
python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name inserting --ordcup_id 1 --marker_id 1
python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name pressing --ordcup_id 1 --nozzle_id 1

python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name reorient_unscrew --bottle_id 6
python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name unscrew_pouring --bottle_id 6 --mugcup_id 1

python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name tool_spoon
python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name tool_funnel

python scripts_kfr/run_Real_Rollout_v2.py --test_id 1 --task_name sweeping --no_interaction --trash_name "yellow pen"

################################################
##### no_interaction = True for BiDemoSyn
################################################
python scripts_kfr/run_Real_Rollout_v2.py --no_interaction --task_name plugpen --marker_id 2
python scripts_kfr/run_Real_Rollout_v2.py --no_interaction --task_name reorient --anyobj_id 1
python scripts_kfr/run_Real_Rollout_v2.py --no_interaction --task_name unscrew --bottle_id 6
python scripts_kfr/run_Real_Rollout_v2.py --no_interaction --task_name pouring --bottle_id 6 --mugcup_id 1
python scripts_kfr/run_Real_Rollout_v2.py --no_interaction --task_name inserting --ordcup_id 1 --marker_id 2
python scripts_kfr/run_Real_Rollout_v2.py --no_interaction --task_name pressing --ordcup_id 1 --nozzle_id 1
...
if __name__ == "__main__":
    ...
    ######################################
    parser.add_argument('--trash_name', default="paper ball", help="trash name for task sweeping")  # [given by user] e.g., paper ball / yellow pen / pink disk
    ...
    if args.task_name in ["unscrew", "pouring", "pressing", "unscrew_pouring", "sweeping"]:   # we aplly YOLO-World + FastSAM / YOLOE
    ...
    if args.task_name == "plugpen":
        ...
        radius_ratio = 0.2; detseg_prompts = "pen,cap"
    if args.task_name == "reorient":
        ...
        radius_ratio = 0.2; detseg_prompts = anyobj_cls  # Note, the rare category spoon/shovel cannot be detected stablely
    if args.task_name == "unscrew":
        ...
        radius_ratio = 0.2; detseg_prompts = "bottle"
    if args.task_name == "pouring":
        ...
        prev_delta_xy_list = [[0, 0, None], [0, 0, None]]
        # vfms_conf = {"yoloworld": 0.5, "fastsam": 0.5}; radius_ratio = 0.2
        radius_ratio = 0.2; detseg_prompts = "cup,bottle"
    ...
    if args.task_name == "inserting":
        ...
        prev_delta_xy_list = [[0, 0, None], [0, 0, None]]
        # vfms_conf = {"yoloworld": 0.5, "fastsam": 0.5}; radius_ratio = 0.2
        radius_ratio = 0.2; detseg_prompts = "cup,pen"  # Note, the up-side-down cup cannot be detected stablely
    if args.task_name == "pressing":
        ...
        prev_delta_xy_list = [[0, 0, None], [0, 0, None]]
        # vfms_conf = {"yoloworld": 0.5, "fastsam": 0.5}; radius_ratio = 0.2
        radius_ratio = 0.2; detseg_prompts = "bottle,cup"
    ...
    if args.task_name == "reorient_unscrew":
        ...
        radius_ratio = 0.2; detseg_prompts = "bottle"  # Note, the lying down bottle cannot be detected stablely
    ...
    if args.task_name == "tool_spoon":
        ...
        radius_ratio = 0.2; detseg_prompts = "spoon,bowl"
    if args.task_name == "tool_funnel":
        ...
        prev_delta_xy_list = [[0, 0, None], [0, 0, None], [0, 0, None]]
        radius_ratio = 0.2; detseg_prompts = "funnel,bottle,cup"  # Note, funnel cannot be detected, we treat it as a bottle

    ######################################
    # actions of these tasks are extraced by one-shot human demonstration
    if args.task_name == "sweeping":
        json_file_name = f"sweeping_01_202507071450_L_eep_new.json"
        saved_seed_img_path = os.path.join(task_folder_dir, "brush-dustpan-paper_ball_seed.jpg")
        seed_ref_pts_dict, test_ref_pts_dict = {"brush":[], "dustpan":[], "trash":[]}, {"brush":[], "dustpan":[], "trash":[]}
        prev_delta_xy_list = [[0, 0, None], [0, 0, None], [0, 0, None]]
        radius_ratio = 0.2; detseg_prompts = "brush,dustpan,paper ball"
     ######################################
    ...
    ############################################################################  
    if args.task_name in ["sweeping"]:
        eef_pose_seq_json_path = os.path.join("./outputs/", args.task_name + "_01", json_file_name)
    else:
    ...
    '''v3: only use Florence2 + SAM2'''
    [x1, y1, x2, y2] = roi_bbox  # for fast and stable detection and segmentation
    left_image_seed_sub = left_image_seed[y1:y2, x1:x2]
    final_res_list, img_vis_seed_cv2 = conduct_object_detect_and_segment(left_image_seed.copy(), prompts_str=detseg_prompts, 
        is_raw_result=True, roi_bbox=cfg_dict["detection_roi_bbox"], roi_bbox_2=cfg_dict["detection_roi_bbox_2"])
    if args.debug_close_loop_vis:
        left_image_vis = left_image_seed.copy(); trash_name = "paper ball" if args.task_name=="sweeping" else None
        left_image_vis[y1:y2, x1:x2] = plot_processed_results_vis(
            left_image_seed_sub.copy(), final_res_list, list(seed_ref_pts_dict.keys()), trash_name=trash_name)
    ...
    for [obj_name, bbox, obj_binary_mask, multi_masks] in final_res_list:
        ...
        if args.task_name in ["plugpen", "reorient", "reorient_unscrew"] or \
            (args.task_name=="inserting" and obj_name=="pen") or (args.task_name=="tool_spoon" and obj_name=="spoon") or \
            (args.task_name=="sweeping" and obj_name=="brush") or (args.task_name=="sweeping" and obj_name=="paper ball"):
            ...
            if args.task_name=="sweeping" and obj_name=="paper ball": obj_name = "trash"
            seed_ref_pts_dict[obj_name] = [c_point_raw, rot_deg_init, bbox]
        if args.task_name in ["unscrew", "pouring", "pressing", "unscrew_pouring", "tool_funnel"] or (args.task_name=="sweeping" and obj_name=="dustpan") or \
        ...
        if args.debug_close_loop_vis:
            ...
            if args.task_name in ["plugpen", "reorient", "reorient_unscrew"] or (args.task_name=="sweeping" and obj_name=="brush") or \
    ...
    if args.task_name == "sweeping":  # close-loop for the right-arm
        # original robot_arms: ['R', 'R', 'R', 'L', 'L', 'L', 'L', 'L', 'R', 'R', 'R', 'R', "L", "L", 'R', 'R', 'L', 'L'] 18 actions
        cl_robot_arms, cl_eef_pose_seqs = robot_arms[:1], eef_pose_seqs[:1]
        robot_arms, eef_pose_seqs = robot_arms[1:], eef_pose_seqs[1:]
    ...
    while True:
        ...
        try:  # at least one manipulated object is detected. we update the final_res_list and im_bgr
            ...
            if args.task_name=="sweeping": detseg_prompts = detseg_prompts.replace("paper ball", args.trash_name)
            final_res_list, img_vis_seed_cv2 = conduct_object_detect_and_segment(left_image_test.copy(), prompts_str=detseg_prompts,                                                      
                is_raw_result=True, roi_bbox=cfg_dict["detection_roi_bbox"], roi_bbox_2=cfg_dict["detection_roi_bbox_2"])
        ...
        if args.debug_close_loop_vis:
            left_image_test_vis = left_image_test.copy(); trash_name = args.trash_name if args.task_name=="sweeping" else None
            left_image_test_vis[y1:y2, x1:x2] = plot_processed_results_vis(
                left_image_test_sub.copy(), final_res_list_temp, list(seed_ref_pts_dict.keys()), trash_name=trash_name )
        ...
        for [obj_name, bbox, obj_binary_mask, multi_masks] in final_res_list_temp:
            ...
            if args.task_name in ["plugpen", "reorient", "reorient_unscrew"] or \
                (args.task_name=="inserting" and obj_name=="pen") or (args.task_name=="tool_spoon" and obj_name=="spoon") or \
                (args.task_name=="sweeping" and obj_name=="brush") or (args.task_name=="sweeping" and obj_name==args.trash_name):
                ...
                if args.task_name=="sweeping" and obj_name==args.trash_name: obj_name = "trash"
        ...
        if args.task_name == "sweeping":
            cur_end_eef_pose[5] += (rot_deg_test - rot_deg_init)  # brush is perpendicularly placed to spoon//shovel/marker-pen
        ...
        if args.task_name in ["plugpen", "reorient", "reorient_unscrew", "tool_spoon", "sweeping"]:
        ...
        if cl_substep_id_record == interp_num-1:  # this close-loop step is finished !!!
            ...
            if args.task_name == "sweeping":
                pt_seeding, pt_testing = reprojected_seeding_pts[1], reprojected_testing_pts[1]  # for the object dustpan
                delta_x1, delta_y1 = pt_testing[0] - pt_seeding[0], pt_testing[1] - pt_seeding[1]
                pt_seeding, pt_testing = reprojected_seeding_pts[2], reprojected_testing_pts[2]  # for the object trash (paper ball / yellow pen)
                delta_x2, delta_y2 = pt_testing[0] - pt_seeding[0], pt_testing[1] - pt_seeding[1]
                eef_pose_seqs[4][0] -= (delta_y1 / 1000); eef_pose_seqs[4][1] -= (delta_x1 / 1000)  # [L] only care the dustpan
                eef_pose_seqs[5][0] -= (delta_y1 / 1000); eef_pose_seqs[5][1] -= (delta_x1 / 1000)  # [L] only care the dustpan
                eef_pose_seqs[6][0] -= (delta_y2 / 1000); eef_pose_seqs[6][1] -= (delta_x2 / 1000)  # [L] care trash (no dustpan)
                eef_pose_seqs[11][0] -= (delta_y2 / 1000); eef_pose_seqs[11][1] -= ( delta_x2 / 1000)  # [L] care trash (no dustpan)
                eef_pose_seqs[7][0] += (delta_y2 / 1000); eef_pose_seqs[7][1] += (delta_x2 / 1000)  # [R] only care the trash
                eef_pose_seqs[8][0] += (delta_y2 / 1000); eef_pose_seqs[8][1] += (delta_x2 / 1000)  # [R] only care the trash
                eef_pose_seqs[9][0] += (delta_y2 / 1000); eef_pose_seqs[9][1] += (delta_x2 / 1000)  # [R] only care the trash
    ...
    for step_id, (robot_arm, eef_pose_seq) in enumerate(zip(robot_arms, eef_pose_seqs)):
        ...
        #######################################
        if args.task_name == "sweeping":  # optimize trajectory of sub-tasks
            if 7 <= step_id <= 9:  # for robot arm R (run sweeping)
                if step_id == 7: trajs_temp.append(robotBase_R.get_current_pose()[0])
                trajs_temp.append(target_pose)
                if step_id < 9: continue
                if step_id == 9: robotBase_R.move_trajectory(trajs_temp, is_cartesian=True, robot_move_type=4); trajs_temp = []; continue
        #######################################
        if robot_arm == "L":
