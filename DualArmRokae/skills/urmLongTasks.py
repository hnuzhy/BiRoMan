...
python skills/urmLongTasks.py --task_name urm_t1_box --rectbox_id 1 --test_id 1
python skills/urmLongTasks.py --task_name urm_t1_box --no_interaction --rectbox_id 1 --test_id 1

python skills/urmLongTasks.py --task_name urm_t2_bowl --cirbowl_id 1 --test_id 1
python skills/urmLongTasks.py --task_name urm_t2_bowl --no_interaction --cirbowl_id 1 --test_id 1

python skills/urmLongTasks.py --task_name urm_t3_basket --basket_id 1 --test_id 1
python skills/urmLongTasks.py --task_name urm_t3_basket --no_interaction --basket_id 1 --test_id 1

python skills/urmLongTasks.py --task_name urm_t4_holder --holder_id 1 --test_id 1
python skills/urmLongTasks.py --task_name urm_t4_holder --no_interaction --holder_id 1 --test_id 1

python skills/urmLongTasks.py --task_name urm_t5_bigjar --bigjar_id 1 --test_id 1
python skills/urmLongTasks.py --task_name urm_t5_bigjar --no_interaction --bigjar_id 1 --test_id 1

python skills/urmLongTasks.py --task_name urm_t6_block --block_id 1 --test_id 1
python skills/urmLongTasks.py --task_name urm_t6_block --no_interaction --block_id 1 --test_id 1
...
if __name__ == "__main__":
    ...
    ###################################################################################
    assert args.rectbox_id >= 1 and args.rectbox_id <= 4, "Please note that rectbox_id is selected from 1 ~ 3 !!!"
    assert args.cirbowl_id >= 1 and args.cirbowl_id <= 4, "Please note that cirbowl_id is selected from 1 ~ 3 !!!"
    assert args.basket_id >= 1 and args.basket_id <= 4, "Please note that basket_id is selected from 1 ~ 3 !!!"
    assert args.holder_id >= 1 and args.holder_id <= 4, "Please note that holder_id is selected from 1 ~ 3 !!!"
    assert args.pencup_id >= 1 and args.pencup_id <= 4, "Please note that pencup_id is selected from 1 ~ 3 !!!"
    assert args.bigjar_id >= 1 and args.bigjar_id <= 4, "Please note that bigjar_id is selected from 1 ~ 3 !!!"
    assert args.block_id >= 1 and args.basket_id <= 4, "Please note that block_id is selected from 1 ~ 3 !!!"
    long_horizon_task_name = args.task_name
    ...
    if long_horizon_task_name == "urm_t1_box":
        cls_name_list = ["box", "box", "box"]  # all three boxes are gray paper boxes
        cls_name = cls_name_list[args.rectbox_id - 1]
        video_name_str = f"{args.task_name}_test{args.test_id}_oid{str(args.rectbox_id).zfill(2)}"
    if long_horizon_task_name == "urm_t2_bowl":
        cls_name_list = ["white bowl", "green bowl", "transparent bowl", "gray bowl"]  # three bowls have different colors and sizes
        cls_name = cls_name_list[args.cirbowl_id - 1]
        video_name_str = f"{args.task_name}_test{args.test_id}_oid{str(args.cirbowl_id).zfill(2)}"
    if long_horizon_task_name == "urm_t3_basket":
        cls_name_list = ["pink basket", "green basket", "blue basket"]  # three baskets have different colors and sizes
        cls_name = cls_name_list[args.basket_id - 1]
        video_name_str = f"{args.task_name}_test{args.test_id}_oid{str(args.basket_id).zfill(2)}"
    if long_horizon_task_name == "urm_t4_holder":
        cls_name_list = ["black cup", "black cup"]  # two pen holders have different sizes
        cls_name = cls_name_list[args.holder_id - 1]
        video_name_str = f"{args.task_name}_test{args.test_id}_oid{str(args.holder_id).zfill(2)}"
        given_state_list = ["C", "B", "B", "A", "A"]  # ["C", "B", "A"] or ["C", "B", "B", "A", "A"]
    if long_horizon_task_name == "urm_t5_bigjar":
        cls_name_list = ["plastic jar", "plastic jar"]  # two big jars have different sizes
        cls_name = cls_name_list[args.bigjar_id - 1]
        video_name_str = f"{args.task_name}_test{args.test_id}_oid{str(args.bigjar_id).zfill(2)}"
        given_state_list = ["A", "B", "C", "B", "A"]  # ["C", "B", "A"] or ["A", "B", "C", "B", "A"]
    if long_horizon_task_name == "urm_t6_block":
        cls_name_list =  ["green block", "purple block", "orange block"]  # three big blocks have different colors
        cls_name = cls_name_list[args.block_id - 1]
        video_name_str = f"{args.task_name}_test{args.test_id}_oid{str(args.block_id).zfill(2)}"
        given_state_list = ["B", "C", "C", "B", "A"]  # ["C", "A"] or ["B", "A"] or ["B", "C", "C", "B", "A"]
    ...
    while True:
        ...
        if long_horizon_task_name == "urm_t1_box":
            final_res_list_dict, cur_state, img_vis_cv2 = processing_test_image_full_stage_urm_t1_box(
            ...
            if cur_state == "A":
                ...
                if (by2 - by1) < (bx2 - bx1):  # standing --> stop (the rectangle box is rightly placed!)
                    is_task_finished = True
                else:  # standing --> single-arm grasping+placing (the rectangle box is not rightly placed!)
                    subtask_name = "grasping_rectbox"; is_task_finished = False
            if cur_state == "B":  # lying down --> dual-arm pivoting
                subtask_name = "pivoting_rectbox"; is_task_finished = False
        ...
        if long_horizon_task_name == "urm_t2_bowl":
            ...
            if cur_state == "A":
                ...
                if bcx > (x2-x1)*(2/5.0) and bcx < (x2-x1)*(3/5.0) and bcy < (y2-y1)*(1/3.0):  # upright --> stop
                    is_task_finished = True
        ...
        if long_horizon_task_name == "urm_t3_basket":
            final_res_list_dict, cur_state, img_vis_cv2 = processing_test_image_full_stage_urm_t3_basket(
                left_image_test.copy(), cls_name, right_image_test.copy())
            if cur_state == "A":
                [bx1, by1, bx2, by2] = final_res_list_dict[cls_name][1]  # [obj_name, bbox, obj_binary_mask, multi_masks]
                bcx, bcy, [x1, y1, x2, y2] = (bx1+bx2)*0.5, (by1+by2)*0.5, cfg_dict["detection_roi_bbox"]
                if bcx > (x2-x1)*(2/5.0) and bcx < (x2-x1)*(3/5.0) and bcy < (y2-y1)*(1/2.0):  # upright --> stop
                    ...
                    subtask_name = "grasping_basket"; is_task_finished = False
            if cur_state == "B":  # inverted --> dual-arm pivoting (the bowl can not be grasped now)
                subtask_name = "flipping_basket"; is_task_finished = False

        if long_horizon_task_name == "urm_t4_holder":
            final_res_list_dict, cur_state, img_vis_cv2 = processing_test_image_full_stage_urm_t4_holder(
                left_image_test.copy(), cls_name, right_image_test.copy())

            if global_substep_id < len(given_state_list):
                cur_state = given_state_list[global_substep_id]
                is_task_finished = False
                if cur_state == "A": subtask_name = "grasping_pencup"  # A --> F
                if cur_state == "B": subtask_name = "grasping_holder"  # B --> A
                if cur_state == "C": subtask_name = "toppling_holder"  # C --> B
            else:
                is_task_finished = True

        if long_horizon_task_name == "urm_t5_bigjar":
            final_res_list_dict, cur_state, img_vis_cv2 = processing_test_image_full_stage_urm_t5_bigjar(
                left_image_test.copy(), cls_name, right_image_test.copy())

            if global_substep_id < len(given_state_list):
                cur_state = given_state_list[global_substep_id]
                is_task_finished = False
                if cur_state == "A": subtask_name = "bilifting_bigjar"  # A --> F
                if cur_state == "B": subtask_name = "pivoting_bigjar"  # B --> A
                if cur_state == "C": subtask_name = "toppling_bigjar"  # C --> B
            else:
                is_task_finished = True

        if long_horizon_task_name == "urm_t6_block":
            final_res_list_dict, cur_state, img_vis_cv2 = processing_test_image_full_stage_urm_t6_block(
                left_image_test.copy(), cls_name, right_image_test.copy())

            if global_substep_id < len(given_state_list):
                cur_state = given_state_list[global_substep_id]
                is_task_finished = False
                if cur_state == "A": subtask_name = "bilifting_block"  # A --> F
                if cur_state == "B": subtask_name = "pivoting_block"  # B --> A
                if cur_state == "C": subtask_name = "flipping_block"  # C --> A
            else:
                is_task_finished = True
        ##########################
        saved_frame_count += 1
        ...
        img_vis_cv2 = img_vis_cv2.astype(np.uint8); obj_bbox = final_res_list_dict[cls_name][1]
        ...
        while True:
            ...
            if args.task_name in ["reorient", "grasping", "flatting", "flipping", "flatting-reorient", "flipping-grasping", 
                "grasping_rectbox", "grasping_cirbowl", "grasping_holder", "grasping_pencup"]:
                if args.task_name in ["reorient", "grasping", "flatting", "flatting-reorient", 
                    "grasping_rectbox", "grasping_cirbowl", "grasping_holder", "grasping_pencup"]:
                    robot_arm_temp = "R" if delta_x >= 0 else "L"
                if args.task_name in ["flipping", "flipping-grasping"]: robot_arm_temp = "R" if pt_testing[2] < 90 else "L"
